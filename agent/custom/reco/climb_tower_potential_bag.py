...
POTENTIAL_XS = [355, 475, 592, 710, 828]
POTENTIAL_W = 118
POTENTIAL_H = 150
...
UPPER_POTENTIAL_FIRST_ROW_Y_OFFSET = 50
UPPER_POTENTIAL_SECOND_ROW_Y_OFFSET = 215
...
class ScreenBagDataProcessor(climb_tower_potential.ScreenDataProcessor):
    ...
    def open_bag(self):
        self.context.run_task("星塔_进入背包界面_agent")
    ...
    def get_potential_name_from_bag(self, roi: list[int]) -> str:
        ...
        if not ocr_results:
            logger.warning("未识别到潜能名称，后续抓取潜能时可能会出现预想外情况")
            return ""
    ...
    def get_potential_recommend_level_from_bag(self, roi: list[int]) -> int:
        ocr_results = self._ocr("星塔_背包_识别推荐等级_agent", "",roi=roi)
        if not ocr_results:
    ...
    def check_potential_recommended_from_bag(self, roi: list[int]) -> bool:
        return self._color("星塔_背包_识别推荐等级_agent",roi=roi)
...
class PotentialReader:
    ...
    def read_potentials(self) -> list[climb_tower_potential.OwnedPotential]:
        ...
        for i in range(3):
            ...
            # 生成潜能列表的roi
            potential_rois, level_rois, recommend_rois = self._generate_rois(first_y, second_y)

            # 然后读取前两行的潜能
            potentials_upper = self._read_rows(potential_rois, level_rois, recommend_rois)
            ...
            if not first_y or not second_y:
                ...
                return []
            potential_rois, level_rois, recommend_rois = self._generate_rois(first_y, second_y)
            potentials_lower = self._read_rows(potential_rois, level_rois, recommend_rois)

            # 识别完一个旅人之后，标记旅人名称
            potentials = potentials_upper + potentials_lower
    ...
    def _read_rows(
            self, potential_rois: list[list[int]], level_rois: list[list[int]], recommend_rois: list[list[int]]
        ...
        for i, roi in enumerate(potential_rois):
            ...
            # 读取核心潜能标记
            potential.core = self.screen.check_potential_recommended_from_bag(roi)
            # 读取旅人名称（由于意义不大所以不读）
            potential.trekker = ""
            # 保存潜能
    ...
    @staticmethod
    def _generate_rois(y1: int, y2: int) -> tuple[list[list[int]], list[list[int]], list[list[int]]]:
        """生成潜能列表的roi"""
        potential_rois = [[POTENTIAL_XS[col], y, POTENTIAL_W, POTENTIAL_H]
                          for y in (y1, y2) for col in range(5)]
        level_rois = [[POTENTIAL_XS[col], y, POTENTIAL_W // 2, POTENTIAL_LEVEL_H]
                          for y in (y1, y2) for col in range(5)]
        recommend_rois = [[POTENTIAL_XS[col] + POTENTIAL_W // 2, y, POTENTIAL_W // 2, POTENTIAL_LEVEL_H]
                          for y in (y1, y2) for col in range(5)]
